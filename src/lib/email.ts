...
function emailLayout({ eyebrow, title, intro, details, action, note }: { eyebrow: string; title: string; intro: string; details?: string; action?: { label: string; href: string }; note?: string }) {
...
function paymentPaidMessage(name: string, amountCents: number, planName: string, paidAt?: Date | null): Message {
  const greeting = firstName(name);
  const details = `${planName}\nPagamento confirmado em ${textDate(paidAt)}\n${formatCurrency(amountCents)}`;

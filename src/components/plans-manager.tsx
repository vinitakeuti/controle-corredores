...
export function PlansManager({ initialPlans }: { initialPlans: Plan[] }) {
  ...
  const [priceDrafts, setPriceDrafts] = useState<Record<string, string>>(() => Object.fromEntries(initialPlans.map((plan) => [plan.id, (plan.priceCents / 100).toFixed(2)])));

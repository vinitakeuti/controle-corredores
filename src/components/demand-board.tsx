...
export function DemandBoard({ areaId, areaName, initialColumns, initialDemands, initialFolders, people, initialMembers, canManageMembers }: { areaId: string; areaName: string; initialColumns: Column[]; initialDemands: Demand[]; initialFolders: Folder[]; people: Person[]; initialMembers: Person[]; canManageMembers: boolean }) {
  ...
  function pointerDown(id: string, target: HTMLButtonElement, pointerId: number, columnId: string) {
    dragStarted.current = false; suppressOpen.current = false;
    target.setPointerCapture(pointerId);
    timer.current = setTimeout(() => { dragStarted.current = true; suppressOpen.current = true; draggingId.current = id; draggingColumn.current = columnId; setDragging(id); navigator.vibrate?.(20); }, 2000);
  }
  function pointerUp(target?: HTMLButtonElement, pointerId?: number) { if (timer.current) clearTimeout(timer.current); timer.current = null; if (dragStarted.current) suppressOpen.current = true; draggingId.current = null; draggingColumn.current = null; if (target && pointerId !== undefined && target.hasPointerCapture(pointerId)) target.releasePointerCapture(pointerId); setDragging(null); }
  function pointerMove(clientX: number, clientY: number) {
    ...
    if (columnId && columnId !== draggingColumn.current) { draggingColumn.current = columnId; void move(id, columnId); }
  }
  ...
  return <section className="demand-board">
    <header className="demand-board-head"><div><p className="eyebrow">Área de trabalho</p><h1>{areaName}.</h1><p>Pressione uma atividade por dois segundos para movê-la entre as colunas.</p></div><div className="demand-board-actions"><div className="demand-settings"><button className="demand-settings-trigger" type="button" aria-label="Opções do quadro" title="Opções do quadro" aria-expanded={boardMenuOpen} onClick={() => setBoardMenuOpen((open) => !open)}>⚙</button>{boardMenuOpen ? <div className="demand-settings-menu"><button type="button" onClick={() => { setBoardMenuOpen(false); void openArchiveList(); }}>Arquivo</button><button type="button" onClick={() => { setBoardMenuOpen(false); setFoldersOpen(true); }}>Pastas</button>{canManageMembers ? <button type="button" onClick={() => { setBoardMenuOpen(false); setManageMembersOpen(true); }}>Pessoas do quadro</button> : null}<button type="button" onClick={() => { setBoardMenuOpen(false); setManageOpen(true); }}>Editar colunas</button></div> : null}</div><button className="button button-dark" type="button" onClick={() => startCreate()}>Nova demanda</button></div></header>
    {message && !createOpen && !manageOpen && !viewing ? <p className="error-message">{message}</p> : null}
    <div className="demand-columns-scroll" aria-label="Quadro de demandas" onPointerMove={(event) => pointerMove(event.clientX, event.clientY)}><div className="demand-columns">{columns.map((column) => <section className={"demand-column " + (dragging ? "is-drop-target" : "")} data-demand-column={column.id} key={column.id}><header><strong>{column.name}</strong><span>{demands.filter((demand) => demand.columnId === column.id).length}</span></header><div className="demand-column-cards">{demands.filter((demand) => demand.columnId === column.id).map((demand) => <button className={"demand-card " + (dragging === demand.id ? "dragging " : "") + (demand.completedAt ? "completed" : "")} key={demand.id} type="button" onPointerDown={(event) => pointerDown(demand.id, event.currentTarget, event.pointerId, demand.columnId)} onPointerMove={(event) => pointerMove(event.clientX, event.clientY)} onPointerUp={(event) => pointerUp(event.currentTarget, event.pointerId)} onPointerCancel={(event) => pointerUp(event.currentTarget, event.pointerId)} onClick={() => { if (suppressOpen.current) { suppressOpen.current = false; return; } setViewing(demand); setMessage(""); }}><strong>{demand.title}</strong>{demand.description ? <p>{demand.description}</p> : null}<small>{demand.completedAt ? "Concluída · " : ""}{dateLabel(demand.scheduledAt)}</small>{demand.assignees.length ? <div className="demand-avatars">{demand.assignees.map(({ user }) => <span title={user.name} key={user.id}>{initials(user.name)}</span>)}</div> : null}</button>)}</div><button className="demand-add-inline" type="button" onClick={() => startCreate(column.id)}>+ Adicionar</button></section>)}</div></div>

...
export function DemandBoard({ areaId, areaName, initialColumns, initialDemands, initialFolders, people, initialMembers, canManageMembers }: { areaId: string; areaName: string; initialColumns: Column[]; initialDemands: Demand[]; initialFolders: Folder[]; people: Person[]; initialMembers: Person[]; canManageMembers: boolean }) {
  ...
  function pointerDown(id: string) {
    dragStarted.current = false; suppressOpen.current = false;
    timer.current = setTimeout(() => { dragStarted.current = true; suppressOpen.current = true; setDragging(id); navigator.vibrate?.(20); }, 2000);
  }
  function pointerUp() { if (timer.current) clearTimeout(timer.current); timer.current = null; if (dragStarted.current) suppressOpen.current = true; setDragging(null); }
  function pointerMove(clientX: number, clientY: number) {
    if (!dragging) return;
    const target = document.elementFromPoint(clientX, clientY)?.closest<HTMLElement>("[data-demand-column]");
    const columnId = target?.dataset.demandColumn;
    if (columnId) void move(dragging, columnId);
  }
  ...
  return <section className="demand-board">
    <header className="demand-board-head"><div><p className="eyebrow">Área de trabalho</p><h1>{areaName}.</h1><p>Pressione uma atividade por dois segundos para movê-la entre as colunas.</p></div><div className="demand-board-actions">{canManageMembers ? <button className="button button-quiet" type="button" onClick={() => setManageMembersOpen(true)}>Pessoas do quadro</button> : null}<button className="button button-quiet" type="button" onClick={() => void openArchiveList()} disabled={saving}>Arquivo</button><button className="button button-quiet" type="button" onClick={() => setManageOpen(true)}>Editar colunas</button><button className="button button-dark" type="button" onClick={() => startCreate()}>Nova demanda</button></div></header>
    {message && !createOpen && !manageOpen && !viewing ? <p className="error-message">{message}</p> : null}
    <div className="demand-columns-scroll" aria-label="Quadro de demandas" onPointerMove={(event) => pointerMove(event.clientX, event.clientY)} onPointerUp={pointerUp} onPointerCancel={pointerUp}><div className="demand-columns">{columns.map((column) => <section className={"demand-column " + (dragging ? "is-drop-target" : "")} data-demand-column={column.id} key={column.id}><header><strong>{column.name}</strong><span>{demands.filter((demand) => demand.columnId === column.id).length}</span></header><div className="demand-column-cards">{demands.filter((demand) => demand.columnId === column.id).map((demand) => <button className={"demand-card " + (dragging === demand.id ? "dragging " : "") + (demand.completedAt ? "completed" : "")} key={demand.id} type="button" onPointerDown={() => pointerDown(demand.id)} onPointerUp={pointerUp} onPointerCancel={pointerUp} onClick={() => { if (suppressOpen.current) { suppressOpen.current = false; return; } setViewing(demand); setMessage(""); }}><strong>{demand.title}</strong>{demand.description ? <p>{demand.description}</p> : null}<small>{demand.completedAt ? "Concluída · " : ""}{dateLabel(demand.scheduledAt)}</small>{demand.assignees.length ? <div className="demand-avatars">{demand.assignees.map(({ user }) => <span title={user.name} key={user.id}>{initials(user.name)}</span>)}</div> : null}</button>)}</div><button className="demand-add-inline" type="button" onClick={() => startCreate(column.id)}>+ Adicionar</button></section>)}</div></div>
    ...
    {viewing ? <DemandView demand={viewing} message={message} saving={saving} folders={folders} archiveOpen={archiveOpen} archiveFolderId={archiveFolderId} setArchiveFolderId={setArchiveFolderId} newFolderName={newFolderName} setNewFolderName={setNewFolderName} close={() => { setViewing(null); setArchiveOpen(false); }} edit={() => { const demand = viewing; setViewing(null); startEdit(demand); }} complete={() => void updateViewed({ completed: !viewing.completedAt })} remove={() => void deleteViewed()} toggleArchive={() => { setArchiveOpen((open) => !open); setMessage(""); }} createFolder={() => void createFolder()} archive={() => void archiveViewed()} /> : null}
...
function DemandView({ demand, message, saving, folders, archiveOpen, archiveFolderId, setArchiveFolderId, newFolderName, setNewFolderName, close, edit, complete, remove, toggleArchive, createFolder, archive }: { demand: Demand; message: string; saving: boolean; folders: Folder[]; archiveOpen: boolean; archiveFolderId: string; setArchiveFolderId: (value: string) => void; newFolderName: string; setNewFolderName: (value: string) => void; close: () => void; edit: () => void; complete: () => void; remove: () => void; toggleArchive: () => void; createFolder: () => void; archive: () => void }) {
  return <div className="demand-editor-layer" role="dialog" aria-modal="true" aria-label="Detalhes da demanda"><button className="demand-editor-backdrop" type="button" onClick={close} aria-label="Fechar" /><section className="demand-editor demand-view"><div className="panel-heading"><div><p className="eyebrow">{demand.completedAt ? "Concluída" : "Demanda"}</p><h2>{demand.title}</h2></div><button className="button button-quiet" type="button" onClick={close}>Fechar</button></div><div className="demand-view-copy">{demand.description ? <p>{demand.description}</p> : <p className="muted">Sem detalhes adicionais.</p>}<dl><div><dt>Data e horário</dt><dd>{dateLabel(demand.scheduledAt)}</dd></div><div><dt>Responsáveis</dt><dd>{demand.assignees.length ? demand.assignees.map(({ user }) => user.name).join(", ") : "Nenhum responsável"}</dd></div></dl></div>{message ? <p className="error-message">{message}</p> : null}<div className="demand-view-actions"><button className="button button-secondary" type="button" onClick={edit}>Editar</button><button className="button button-secondary" type="button" onClick={complete} disabled={saving}>{demand.completedAt ? "Reabrir" : "Concluir"}</button><button className="button button-secondary" type="button" onClick={toggleArchive}>Arquivar</button><button className="button button-danger-quiet" type="button" onClick={remove} disabled={saving}>Excluir</button></div>{archiveOpen ? <div className="demand-archive-panel"><p className="eyebrow">Arquivo</p><h3>Escolha uma pasta</h3><div className="field"><label htmlFor="demand-folder">Pasta</label><select id="demand-folder" value={archiveFolderId} onChange={(event) => setArchiveFolderId(event.target.value)}><option value="">Selecione uma pasta</option>{folders.map((folder) => <option key={folder.id} value={folder.id}>{folder.name}</option>)}</select></div><div className="demand-folder-create"><input aria-label="Nome da nova pasta" placeholder="Nova pasta" value={newFolderName} onChange={(event) => setNewFolderName(event.target.value)} /><button className="button button-quiet" type="button" onClick={createFolder} disabled={saving}>Criar pasta</button></div><button className="button button-dark" type="button" onClick={archive} disabled={saving}>Arquivar demanda</button></div> : null}</section></div>;
}

...
export function DemandBoard({ areaId, areaName, initialColumns, initialDemands, people }: { areaId: string; areaName: string; initialColumns: Column[]; initialDemands: Demand[]; people: Person[] }) {
  ...
  async function deleteColumn(column: Column) {
    ...
    setColumns((items) => items.filter((item) => item.id !== column.id)); setMessage("");
  }

  return <section className="demand-board">
    <header className="demand-board-head"><div><p className="eyebrow">Área de trabalho</p><h1>{areaName}.</h1><p>Pressione uma atividade por dois segundos para movê-la entre as colunas.</p></div><div className="demand-board-actions"><button className="button button-quiet" type="button" onClick={() => setManageOpen(true)}>Editar colunas</button><button className="button button-dark" type="button" onClick={() => startCreate()}>Nova demanda</button></div></header>

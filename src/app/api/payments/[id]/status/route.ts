...
import { synchronizeAppmaxOrder, synchronizeAsaasPayment } from "@/lib/payment-service";
...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const payment = await prisma.payment.findUnique({
    where: { id },
    select: { id: true, userId: true, status: true, provider: true, providerOrderId: true, updatedAt: true },
  });
  if (!payment || payment.userId !== user.id) {
    return NextResponse.json({ error: "Pagamento não encontrado" }, { status: 404, headers: noStoreHeaders() });
  }

  const maySynchronize = payment.updatedAt <= new Date(Date.now() - 30_000);
  if (payment.status === "PENDING" && payment.providerOrderId && maySynchronize) {
    try {
      const synchronized = payment.provider === "ASAAS"
        ? await synchronizeAsaasPayment(payment.providerOrderId)
        : await synchronizeAppmaxOrder(payment.providerOrderId);

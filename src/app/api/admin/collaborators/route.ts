...
export async function GET() {
  ...
  const collaborators = await prisma.user.findMany({ where: { role: { in: [...staffRoles] } }, select: { id: true, name: true, email: true, role: true, active: true, joinedAt: true, commissionRateBps: true }, orderBy: [{ role: "asc" }, { name: "asc" }] });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const collaborator = await prisma.user.create({ data: { name, email, role, commissionRateBps: rateBps, passwordHash: await bcrypt.hash(temporaryPassword, 12), passwordIsTemporary: true }, select: { id: true, name: true, email: true, role: true, active: true, joinedAt: true, commissionRateBps: true } });
    let emailSent = false;
    try {
...
export async function PATCH(request: Request) {
  if (!isSameOrigin(request)) return NextResponse.json({ error: "Origem inválida" }, { status: 403, headers: noStoreHeaders() });
  const admin = await getCurrentUser();
  if (!admin || admin.role !== UserRole.ADMIN) return NextResponse.json({ error: "Apenas administradores podem definir comissões" }, { status: 403, headers: noStoreHeaders() });
  try {
    const body = await request.json() as { collaboratorId?: unknown; commissionRateBps?: unknown };
    ...
    const collaborator = await prisma.user.updateMany({ where: { id: collaboratorId, role: { in: [...staffRoles] } }, data: { commissionRateBps: rateBps } });
    if (!collaborator.count) return NextResponse.json({ error: "Colaborador não encontrado." }, { status: 404, headers: noStoreHeaders() });
    return NextResponse.json({ commissionRateBps: rateBps }, { headers: noStoreHeaders() });
  } catch {

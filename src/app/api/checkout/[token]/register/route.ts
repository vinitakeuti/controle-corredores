...
export async function POST(request: Request, context: { params: Promise<{ token: string }> }) {
  try {
    ...
    const link = await prisma.paymentLink.findUnique({ where: { tokenHash: hashOpaqueToken(token) }, select: { id: true, status: true, userId: true, createdById: true, planId: true, planName: true, amountCents: true, allowedMethods: true, plan: { select: { priceCents: true, period: true, allowedMethods: true, automaticPixEnabled: true } } } });
    ...
    const student = await prisma.$transaction(async (transaction) => {
      const created = await transaction.user.create({ data: { name, email, phone, cpf, birthDate, passwordHash, role: UserRole.STUDENT, saleOwnerId: link.createdById, passwordIsTemporary: false, liabilityTermRequiredAt: new Date() } });
      await transaction.subscription.create({ data: { userId: created.id, status: SubscriptionStatus.INCOMPLETE, planId: link.planId, planName: link.planName, priceCents: link.plan?.priceCents ?? link.amountCents, billingPeriod: link.plan?.period ?? "MONTHLY", allowedMethods: link.plan?.allowedMethods ?? link.allowedMethods, automaticPixEnabled: link.plan?.automaticPixEnabled ?? true } });

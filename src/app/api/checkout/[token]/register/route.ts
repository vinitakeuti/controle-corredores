...
export async function POST(request: Request, context: { params: Promise<{ token: string }> }) {
  try {
    ...
    const link = await prisma.paymentLink.findUnique({ where: { tokenHash: hashOpaqueToken(token) }, select: { id: true, status: true, userId: true, planId: true, planName: true, amountCents: true, allowedMethods: true, plan: { select: { priceCents: true, period: true, allowedMethods: true, automaticPixEnabled: true } } } });
    ...
    const student = await prisma.$transaction(async (transaction) => {
      const created = await transaction.user.create({ data: { name, email, phone, cpf, birthDate, passwordHash, role: UserRole.STUDENT, passwordIsTemporary: false, liabilityTermRequiredAt: new Date() } });

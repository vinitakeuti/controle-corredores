    ...
    <AppShell user={user} current="admin">
      <FinancialValues>
        ...
        <div className="section-grid">
          <section className="panel"><div className="panel-heading"><div><h2>Pagamentos de {currentMonth}</h2><p>Confirmações registradas neste mês.</p></div></div>{recentPayments.length ? <div className="notification-list">{recentPayments.map((payment) => <div className="notification-item" key={payment.id}><span className="notification-mark">OK</span><div><div className="notification-title">{payment.user.name}</div><div className="notification-description">{paymentMethodLabel(payment.method)} · <span className="money-value">{formatCurrency(payment.amountCents)}</span></div></div><time className="notification-time">{formatTime(payment.paidAt ?? payment.createdAt)}</time></div>)}</div> : <div className="empty-state">Ainda não há pagamentos confirmados neste mês.</div>}</section>

...
export default async function SalesAnalysisPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const performance = staff.map((seller) => {
    ...
    return { ...seller, total: records.length, approved: won.length, pending: records.length - won.length, invoicedRevenue, monthlyRevenue, commission: commissionFor(monthlyRevenue, seller.commissionRateBps) };
  }).filter((seller) => seller.total > 0 || user.role === UserRole.OPERATOR).sort((left, right) => right.monthlyRevenue - left.monthlyRevenue || right.approved - left.approved || left.name.localeCompare(right.name, "pt-BR"));
  ...
  return <AppShell user={user} current="sales"><header className="page-heading sales-analysis-heading"><div><p className="eyebrow">Comercial</p><h1>Analisar vendas.</h1><p>{user.role === UserRole.ADMIN ? "Faturamento, valor mensal, conversão e comissões da equipe comercial." : "Acompanhe suas vendas aprovadas e sua comissão no período."}</p></div><Link className="directory-action" href="/admin/vendas">← Voltar para vendas</Link></header><nav className="sales-period-tabs" aria-label="Selecionar período">{periods.map((item) => <Link className={item.id === period ? "active" : ""} href={`/admin/vendas/analise?period=${item.id}`} key={item.id}>{item.label}</Link>)}</nav><p className="sales-range-label">{formatDate(range.from)} — {formatDate(range.to)}</p><section className="metric-grid sales-metrics sales-analysis-metrics"><article className="metric-card"><p>Vendas aprovadas</p><strong>{totalApproved}</strong><small>primeiro pagamento confirmado</small></article><article className="metric-card"><p>Faturamento</p><strong>{formatCurrency(totalInvoicedRevenue)}</strong><small>valor total recebido nas vendas</small></article><article className="metric-card"><p>Valor mensal</p><strong>{formatCurrency(totalMonthlyRevenue)}</strong><small>primeira parcela de cada venda</small></article><article className="metric-card accent"><p>Comissões</p><strong>{formatCurrency(totalCommission)}</strong><small>{user.role === UserRole.ADMIN ? "somente sobre a primeira parcela" : "estimativa sobre a primeira parcela"}</small></article></section><section className="panel sales-analysis-panel"><div className="panel-heading"><div><p className="eyebrow">{user.role === UserRole.ADMIN ? "Ranking comercial" : "Meu resultado"}</p><h2>{user.role === UserRole.ADMIN ? "Desempenho por vendedor" : user.name}</h2><p>Comissão calculada somente sobre a parcela do primeiro mês de cada venda aprovada.</p></div></div>{performance.length ? <div className="sales-analysis-list">{performance.map((seller, index) => <article className="sales-analysis-row" key={seller.id}><div className="sales-rank">{user.role === UserRole.ADMIN ? String(index + 1).padStart(2, "0") : "EU"}</div><div className="sales-person"><strong>{seller.name}</strong><span>{seller.email}</span><small>Comissão definida: {rateLabel(seller.commissionRateBps)}</small></div><dl><div><dt>Vendas</dt><dd>{seller.approved} aprovadas <span>· {seller.pending} pendentes</span></dd></div><div><dt>Faturado</dt><dd>{formatCurrency(seller.invoicedRevenue)}</dd></div><div><dt>Valor mensal</dt><dd>{formatCurrency(seller.monthlyRevenue)}</dd></div><div className="sales-commission-value"><dt>Comissão</dt><dd>{formatCurrency(seller.commission)}</dd></div></dl></article>)}</div> : <div className="empty-state">Nenhuma venda atribuída neste período.</div>}</section></AppShell>;
}

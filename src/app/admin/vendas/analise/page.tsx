...
export default async function SalesAnalysisPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const [students, staff] = await Promise.all([
    prisma.user.findMany({ where: salesScope, select: { id: true, saleOwnerId: true, payments: { where: { status: PaymentStatus.PAID }, orderBy: { paidAt: "asc" }, take: 1 } } }),
  ...
  const performance = staff.map((seller) => {
    ...
    const revenue = won.reduce((sum, student) => sum + (student.payments[0]?.amountCents ?? 0), 0);
    return { ...seller, total: records.length, approved: won.length, pending: records.length - won.length, revenue, commission: commissionFor(revenue, seller.commissionRateBps) };
  }).filter((seller) => seller.total > 0 || user.role === UserRole.OPERATOR).sort((left, right) => right.revenue - left.revenue || right.approved - left.approved || left.name.localeCompare(right.name, "pt-BR"));
  const totalRevenue = performance.reduce((sum, seller) => sum + seller.revenue, 0);
  ...
  return <AppShell user={user} current="sales"><header className="page-heading sales-analysis-heading"><div><p className="eyebrow">Comercial</p><h1>Analisar vendas.</h1><p>{user.role === UserRole.ADMIN ? "Receita, conversão e comissões da equipe comercial." : "Acompanhe suas vendas aprovadas e sua comissão no período."}</p></div><Link className="directory-action" href="/admin/vendas">← Voltar para vendas</Link></header><nav className="sales-period-tabs" aria-label="Selecionar período">{periods.map((item) => <Link className={item.id === period ? "active" : ""} href={`/admin/vendas/analise?period=${item.id}`} key={item.id}>{item.label}</Link>)}</nav><p className="sales-range-label">{formatDate(range.from)} — {formatDate(range.to)}</p><section className="metric-grid sales-metrics sales-analysis-metrics"><article className="metric-card"><p>Vendas aprovadas</p><strong>{totalApproved}</strong><small>primeiro pagamento confirmado</small></article><article className="metric-card"><p>Receita aprovada</p><strong>{formatCurrency(totalRevenue)}</strong><small>vendas atribuídas no período</small></article><article className="metric-card accent"><p>Comissões</p><strong>{formatCurrency(totalCommission)}</strong><small>{user.role === UserRole.ADMIN ? "total estimado da equipe" : "estimativa da sua comissão"}</small></article></section><section className="panel sales-analysis-panel"><div className="panel-heading"><div><p className="eyebrow">{user.role === UserRole.ADMIN ? "Ranking comercial" : "Meu resultado"}</p><h2>{user.role === UserRole.ADMIN ? "Desempenho por vendedor" : user.name}</h2><p>Comissão calculada sobre a receita aprovada de cada venda atribuída.</p></div></div>{performance.length ? <div className="sales-analysis-list">{performance.map((seller, index) => <article className="sales-analysis-row" key={seller.id}><div className="sales-rank">{user.role === UserRole.ADMIN ? String(index + 1).padStart(2, "0") : "EU"}</div><div className="sales-person"><strong>{seller.name}</strong><span>{seller.email}</span><small>Comissão definida: {rateLabel(seller.commissionRateBps)}</small></div><dl><div><dt>Vendas</dt><dd>{seller.approved} aprovadas <span>· {seller.pending} pendentes</span></dd></div><div><dt>Receita</dt><dd>{formatCurrency(seller.revenue)}</dd></div><div className="sales-commission-value"><dt>Comissão</dt><dd>{formatCurrency(seller.commission)}</dd></div></dl></article>)}</div> : <div className="empty-state">Nenhuma venda atribuída neste período.</div>}</section></AppShell>;
}

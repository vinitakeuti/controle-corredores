...
export default async function SalesAnalysisPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const canSeeSalesValues = isAdmin || user.role === UserRole.OPERATOR;

  return <AppShell user={user} current="sales"><header className="page-heading sales-analysis-heading"><div><p className="eyebrow">Comercial</p><h1>Analisar vendas.</h1><p>{isAdmin ? "Valor das vendas, valor mensal, conversão e comissões da equipe comercial." : user.role === UserRole.OPERATOR ? "Acompanhe suas vendas aprovadas e sua comissão no período." : "Acompanhe as conversões da equipe no período."}</p></div><Link className="directory-action" href="/admin/vendas">← Voltar para vendas</Link></header><nav className="sales-period-tabs" aria-label="Selecionar período">{periods.map((item) => <Link className={item.id === period ? "active" : ""} href={`/admin/vendas/analise?period=${item.id}`} key={item.id}>{item.label}</Link>)}</nav><p className="sales-range-label">{formatDate(range.from)} — {formatDate(range.to)}</p><section className="metric-grid sales-metrics sales-analysis-metrics"><article className="metric-card"><p>Vendas aprovadas</p><strong>{totalApproved}</strong><small>primeiro pagamento confirmado</small></article>{isAdmin ? <><article className="metric-card"><p>Valor das vendas</p><strong>{formatCurrency(totalInvoicedRevenue)}</strong><small>valor total de cada venda aprovada</small></article><article className="metric-card"><p>Valor mensal</p><strong>{formatCurrency(totalMonthlyRevenue)}</strong><small>equivalente mensal das vendas</small></article></> : null}{canSeeSalesValues ? <article className="metric-card accent"><p>Comissões</p><strong>{formatCurrency(totalCommission)}</strong><small>calculadas sobre o valor total das vendas</small></article> : null}</section><section className="panel sales-analysis-panel"><div className="panel-heading"><div><p className="eyebrow">{isAdmin ? "Ranking comercial" : user.role === UserRole.OPERATOR ? "Meu resultado" : "Acompanhamento comercial"}</p><h2>{isAdmin ? "Desempenho por vendedor" : user.role === UserRole.OPERATOR ? user.name : "Vendas no período"}</h2><p>{canSeeSalesValues ? "Comissão calculada sobre o valor total de cada venda aprovada." : "Acompanhe as vendas aprovadas e pendentes por vendedor."}</p></div></div>{performance.length ? <div className="sales-analysis-list">{performance.map((seller, index) => <article className="sales-analysis-row" key={seller.id}><div className="sales-rank">{isAdmin ? String(index + 1).padStart(2, "0") : user.role === UserRole.OPERATOR ? "EU" : String(index + 1).padStart(2, "0")}</div><div className="sales-person"><strong>{seller.name}</strong><span>{seller.email}</span>{canSeeSalesValues ? <small>Comissão definida: {rateLabel(seller.commissionRateBps)}</small> : null}</div><dl><div><dt>Vendas</dt><dd>{seller.approved} aprovadas <span>· {seller.pending} pendentes</span></dd></div>{canSeeSalesValues ? <><div><dt>Valor das vendas</dt><dd>{formatCurrency(seller.invoicedRevenue)}</dd></div><div><dt>Valor mensal</dt><dd>{formatCurrency(seller.monthlyRevenue)}</dd></div><div className="sales-commission-value"><dt>Comissão</dt><dd>{formatCurrency(seller.commission)}</dd></div></> : null}</dl></article>)}</div> : <div className="empty-state">Nenhuma venda atribuída neste período.</div>}</section></AppShell>;
}

...
export default async function SettingsPage() {
  ...
  const collaborators = await prisma.user.findMany({ where: { role: { in: [UserRole.ADMIN, UserRole.OPERATOR] } }, select: { id: true, name: true, email: true, role: true, active: true, joinedAt: true, commissionRateBps: true }, orderBy: [{ role: "asc" }, { name: "asc" }] });

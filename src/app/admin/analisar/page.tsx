...
type ListStatus = "on-time" | "overdue";
const PAGE_SIZE = 12;

function getListStatus(value?: string): ListStatus {
  return value === "overdue" ? "overdue" : "on-time";
}
...
export default async function AnalyzePage({ searchParams }: { searchParams: Promise<AnalyzeSearchParams> }) {
  ...
  const [onTimeTotal, overdueTotal] = await Promise.all([prisma.user.count({ where: { ...onTimeWhere, ...searchFilter } }), prisma.user.count({ where: { ...overdueWhere, ...searchFilter } })]);
  const selectedTotal = status === "overdue" ? overdueTotal : onTimeTotal;
  const totalPages = Math.max(1, Math.ceil(selectedTotal / PAGE_SIZE));
  const currentPage = Math.min(requestedPage, totalPages);
  const students = await prisma.user.findMany({
    where: status === "overdue" ? overdueWhere : onTimeWhere,
    ...
    <AppShell user={user} current="analysis">
      ...
      <section className="panel analysis-panel">
        ...
        <div className="analysis-toolbar">
          <div><h2>{status === "overdue" ? "Alunos em atraso" : "Alunos em dia"}</h2><p>Página {currentPage} de {totalPages}, com {PAGE_SIZE} registros por vez.</p></div>
        ...
        <div className="students-table-shell">
          <table className="students-table">
            ...
            <tbody>
              {students.length === 0 ? <tr><td className="table-empty" colSpan={3}>Nenhum aluno encontrado.</td></tr> : students.map((student) => <tr key={student.id}><td><div className="student-primary">{student.name}</div><div className="student-secondary">{student.email}</div></td><td>{status === "overdue" ? "Venceu em " : "Próxima em "}{formatDate(student.subscription?.nextBillingAt)}</td><td><span className={`pill ${status === "overdue" ? "pill-coral" : ""}`}>{subscriptionLabel(student.subscription?.status ?? "")}</span></td></tr>)}
            </tbody>
          </table>
        </div>

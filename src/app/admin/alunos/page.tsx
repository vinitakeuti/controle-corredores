...
export default async function StudentsPage({ searchParams }: { searchParams: Promise<StudentsSearchParams> }) {
  ...
  const students = await prisma.user.findMany({ where, include: { subscriptions: { orderBy: { nextBillingAt: "asc" }, take: 1 }, payments: { where: { status: "PAID" }, orderBy: { paidAt: "desc" }, take: 1 } }, orderBy: { name: "asc" }, skip: (currentPage - 1) * PAGE_SIZE, take: PAGE_SIZE });
